"""
Iterative Pseudo-label Refinement: Hard Margin + Raw-DINO RKD + Class-Balanced High-Purity MemBank
==================================================================================================

Changes in this version:
1. RKD now distills raw DINOv2 pairwise geometry instead of the random initial projection head.
2. The old-class memory bank is class-balanced: each old pseudo-class gets its own fixed-size queue/top-confidence buffer.
"""
...
ROUNDS    = 2  # Capped at 2 since peak is hit early
EPOCHS_0  = 100
EPOCHS_R  = 50
ITERS_PER_EPOCH = 20
LR_0, LR_R = 3e-4, 1e-4
...
# Raw-DINO relational distillation weight.
# 0.1–0.3 is usually safer than 1.0 because it preserves old geometry
# without preventing the projection head from improving separability.
W_DISTILL      = 0.2

# Class-balanced old memory bank.
MEM_PER_OLD_CLASS = 64
MEM_MIN_WEIGHT    = 0.75
...
print(f"  A:{N_A}  B:{N_B}  novel(B):{is_novel_B.sum()}  K_NEW:{K_NEW}")
...
# ──────────────────────────────────────────────────────────────────────────────
# Network, Class-Balanced Memory Bank & SoftMatch
# ──────────────────────────────────────────────────────────────────────────────
...
class ClassBalancedMemoryBank:
    """
    Per-class high-purity memory bank for old pseudo-classes.

    Each old class has exactly `per_class` slots. If the class bank is not full,
    new samples are inserted. Once full, a new sample replaces the lowest-weight
    existing sample only if its confidence/weight is higher.

    This prevents the old anchor from being dominated by easy/frequent classes.
    """
    def __init__(self, num_classes, per_class, dim, device):
        self.num_classes = num_classes
        self.per_class   = per_class
        self.dim         = dim
        self.device      = device

        self.features = torch.zeros(num_classes, per_class, dim, device=device)
        self.weights  = torch.zeros(num_classes, per_class, device=device)
        self.valid    = torch.zeros(num_classes, per_class, dtype=torch.bool, device=device)
    ...
    def enqueue(self, feats, labels, weights):
        ...
        weights = weights.detach().float().to(self.device)
        ...
        for i in range(feats.size(0)):
            ...
            w = weights[i]
            cnt = int(self.counts[k].item())

            if cnt < self.per_class:
                slot = cnt
                self.features[k, slot] = feats[i]
                self.weights[k, slot]  = w
                self.valid[k, slot]    = True
                self.counts[k] += 1
            else:
                min_slot = torch.argmin(self.weights[k]).item()
                if w > self.weights[k, min_slot]:
                    self.features[k, min_slot] = feats[i]
                    self.weights[k, min_slot]  = w
                    self.valid[k, min_slot]    = True

    def get_all(self):
        flat_valid = self.valid.reshape(-1)
        if not flat_valid.any():
            return None, None, None

        flat_features = self.features.reshape(-1, self.dim)
        flat_weights  = self.weights.reshape(-1)
        label_grid = torch.arange(self.num_classes, device=self.device).view(-1, 1)
        label_grid = label_grid.expand(self.num_classes, self.per_class).reshape(-1)

        return flat_features[flat_valid], label_grid[flat_valid], flat_weights[flat_valid]
...
def supcon_loss(z, y_old_batch, w_old_batch,
    ...
    # ── AA term: A + confident old-B + class-balanced memory bank ─────────────
    if n_a_o > 0:
        ...
        if N_AA > 0 and N_ALL > 1:
            ...
            has_aa = aa_sum > 0
            if has_aa.any():
                l_aa = -((pair_w * lp_aa).sum(dim=1)[has_aa] /
                         aa_sum[has_aa].clamp_min(1e-8))
    ...
    if (bb_sum > 0).any() and w_bb > 0:
        has = bb_sum > 0
        l_bb = -((bb_w * lp).sum(dim=1)[has] /
                 bb_sum[has].clamp_min(1e-8))
    ...
    if w_nn > 0 and n_cn > 1:
        ...
        if (nn_sum > 0).any():
            has = nn_sum > 0
            l_nn = -((nn_w * lp).sum(dim=1)[has] /
                     nn_sum[has].clamp_min(1e-8))
...
# ──────────────────────────────────────────────────────────────────────────────
# Unsupervised Constrained Clustering
# ──────────────────────────────────────────────────────────────────────────────
...
def make_constrained_joint_labels(Z_anchor_A, Z_anchor_B, hard_y_A, target_max_p=0.7):
    ...
    try:
        ...
        ax.hist(max_sim_B, bins=80, density=True, alpha=0.4,
                color='gray', label='all B')
        ax.axvline(threshold, color='red', linewidth=2,
                   label=f'1D KMeans threshold={threshold:.3f}')
        ax.axvline(low_c,  color='orange', linestyle='--',
                   label=f'low center={low_c:.3f}')
        ax.axvline(high_c, color='blue', linestyle='--',
                   label=f'high center={high_c:.3f}')

        novel_sims = max_sim_B[is_novel_B]
        old_sims   = max_sim_B[~is_novel_B]
        ax.hist(novel_sims, bins=80, density=True, alpha=0.4,
                color='orange', label='true novel')
        ax.hist(old_sims,   bins=80, density=True, alpha=0.4,
                color='blue', label='true old')

        est_novel_frac = novel_mask_B.mean()
        true_novel_frac = is_novel_B.mean()
        ax.set_title(
            f'max_sim_B distribution  |  '
            f'est novel={est_novel_frac:.1%}  true novel={true_novel_frac:.1%}'
        )
        ax.legend(fontsize=8)
        fig.tight_layout()
        fig.savefig('max_sim_distribution.png', dpi=150)
        plt.close(fig)
        print(f"  Threshold diagnostic saved → max_sim_distribution.png")
        print(f"  1D KMeans: low_c={low_c:.3f}  high_c={high_c:.3f}  "
              f"threshold={threshold:.3f}")
        print(f"  Novel mask: est={est_novel_frac:.1%}  true={true_novel_frac:.1%}  "
              f"overlap={((novel_mask_B) & (~is_novel_B)).sum()} false positives  "
              f"{((~novel_mask_B) & is_novel_B).sum()} false negatives")
    except Exception as e:
        print(f"  Diagnostic plot skipped: {e}")

    # ── Novel prototype initialization ───────────────────────────────────────
    if int(novel_mask_B.sum()) >= K_NOV_LOC:
        km_nov = KMeans(n_clusters=K_NOV_LOC, n_init=15,
                        random_state=SEED).fit(Z_B[novel_mask_B])
        novel_protos = normalize(km_nov.cluster_centers_,
                                 norm="l2").astype(np.float32)
    else:
    ...
    # ── Constrained EM-like refinement: keep old prototypes anchored; update novel only ──
    for it in range(20):
        sim    = Z_AB @ centers.T
    ...
    # ── Silhouette margin: drop boundary B samples ───────────────────────────
    ...
    is_confident_B = margin_B > conf_thresh

    hard_B = hard_AB[len(Z_A):]

    crit1_novel_B = hard_B >= K_OLD
    confident_novel_B = crit1_novel_B & novel_mask_B & is_confident_B

    crit1_old_B = hard_B < K_OLD
    safe_old_thresh = (threshold + high_c) / 2.0
    crit2_old_B = max_sim_B > safe_old_thresh
    confident_old_B = crit1_old_B & crit2_old_B & is_confident_B
...
print("\n" + "="*72)
print("ITERATIVE REFINEMENT (Hard Margin + Raw-DINO RKD + Class-Balanced MemBank)")
print("="*72)
...
for rnd in range(ROUNDS):
    ...
    hard_y, soft_p, centers, used_T, ach_max = make_pseudo_labels(Z_A_current, target_max_p=0.7)
    ...
    if rnd == 0:
        ...
        mem_bank = ClassBalancedMemoryBank(
            num_classes=K_OLD,
            per_class=MEM_PER_OLD_CLASS,
            dim=128,
            device=DEVICE,
        )
    ...
    for ep in range(epochs):
        ...
        for _ in range(ITERS_PER_EPOCH):
            ...
            # ── Raw-DINO Relational Knowledge Distillation ───────────────────
            # Preserve pairwise geometry of raw DINOv2 A embeddings, not the
            # geometry of a randomly initialized projection head.
            if n_a > 1:
                with torch.no_grad():
                    x_raw_A = F.normalize(x_batch[:n_a], dim=-1)
                    sim_orig = x_raw_A @ x_raw_A.T

                sim_new = z[:n_a] @ z[:n_a].T
                offdiag = ~torch.eye(n_a, dtype=torch.bool, device=DEVICE)
                l_distill = F.mse_loss(sim_new[offdiag], sim_orig[offdiag])
            else:
            ...
            # ── Class-Balanced High-Purity Memory Bank ───────────────────────
            with torch.no_grad():
                z_teacher = teacher(x_batch[:n_a])
                valid_mask = conf_weight >= MEM_MIN_WEIGHT
                if valid_mask.any():
                    mem_bank.enqueue(
        ...
        if (ep + 1) % 25 == 0 or ep == 0 or ep == epochs - 1:
            counts_now = mem_bank.class_counts()
            print(f"    ep {ep+1:>3}/{epochs}  "
                  f"mem filled={counts_now.sum():>4}/{K_OLD * MEM_PER_OLD_CLASS}  "
                  f"min/class={counts_now.min():>2}  max/class={counts_now.max():>2}")

    print(f"  Round {rnd} training done in {time.time()-t0:.1f}s")
    ...
    Z_B_fused = normalize(Z_B, norm="l2")
    res_fuse = gcd_acc(Z_B_fused, f"Round {rnd} — pure (no fusion)", verbose=False)
    all_results.append((f"Round {rnd}  pure", *res_fuse))
