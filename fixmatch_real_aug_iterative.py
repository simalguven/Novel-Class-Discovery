...
EPOCHS_0     = 100
EPOCHS_R     = 50
FIXMATCH_TAU       = 0.70     # final threshold; 0.95 was too strict for 50 pseudo-clusters
FIXMATCH_TAU_START = 0.35     # warmup threshold while the pseudo-cluster classifier calibrates
FIXMATCH_TAU_WARM  = 20       # epochs to ramp START -> final TAU
FIXMATCH_MIN_KEEP  = 8        # keep at least this many anchor A samples per step by cluster-confidence
USE_CLUSTER_TARGET_FM = True   # use K-Means pseudo-label as the strong-view target
LAMBDA_FM          = 0.5      # weak→strong consistency CE
LAMBDA_ANCH        = 1.0      # CE to round-level cluster pseudo-labels; classifier calibration
LAMBDA_A_SUP       = 1.0      # A pseudo-label SupCon on weak+strong views
LAMBDA_B_AUG       = 0.2      # B same-image weak↔strong contrastive consistency
SUPCON_TAU         = 0.1
CE_TAU             = 1.0      # temperature before classifier softmax
CLASSIFIER_LOGIT_SCALE = 5.0  # helps 50-way classifier produce usable confidence
...
def align_centers_and_labels(new_centers: np.ndarray,
                             ...
                             prev_centers: Optional[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
    """Align K-Means cluster ids to the previous round when spaces match.

    Round 0 uses UMAP-10 centers, while round 1 clusters 128-D projected
    features. Those center matrices are intentionally incompatible, so alignment
    must be skipped for the first UMAP→projection transition. Alignment resumes
    from round 2 onward, when both previous and current centers are 128-D.
    """
    if prev_centers is None:
        return new_centers, new_labels
    if prev_centers.shape[1] != new_centers.shape[1]:
        print(f"       Skipping centroid alignment: previous dim={prev_centers.shape[1]}, current dim={new_centers.shape[1]}")
        return new_centers, new_labels
    ...
    # aligned_centers[old_id] = new center that best matches old_id
...
def cluster_and_confidence(Z_A_current: np.ndarray,
                           prev_centers: Optional[np.ndarray]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    centers, pseudo_y = align_centers_and_labels(centers, pseudo_y, prev_centers)
...
def main() -> None:
    ...
    for rnd in range(ROUNDS):
        ...
        pseudo_y, centers50, conf, tier = cluster_and_confidence(Z_A_current, prev_centers)
        ...
        set_lora_trainable(model.backbone, not (FREEZE_BACKBONE_R0 and rnd == 0))
        param_groups = [
            {"params": list(model.head.parameters()) + list(model.classifier.parameters()), "lr": lr_head, "weight_decay": 1e-4},
        ]
        bb_params = lora_parameters(model.backbone)
        if bb_params:
            param_groups.append({"params": bb_params, "lr": lr_bb, "weight_decay": 0.0})
        opt = torch.optim.AdamW(param_groups)
        sch = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=epochs)

        print(f"  [3] Training {epochs} epochs  lr_head={lr_head:.0e}  lr_lora={lr_bb:.0e}  tau={FIXMATCH_TAU_START:.2f}→{FIXMATCH_TAU:.2f}")
        t0 = time.time()
