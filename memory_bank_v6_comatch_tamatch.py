"""
Iterative Pseudo-label Refinement: Hard Margin + RKD + Class-Balanced High-Purity MemBank
=========================================================================================

This version reverts the distillation path to the provided RKD implementation and changes
only the memory bank mechanism:
- RKD is unchanged: initial_teacher -> pairwise MSE on projected A similarities.
- Memory bank is now class-balanced across old pseudo-classes.
"""
...
#W_NN_SCHEDULE  = [0.0, 0.35, 0.45, 0.45, 0.45, 0.45]
#W_NA_SCHEDULE  = [0.0, 0.25, 0.15, 0.05, 0.0, 0.0]
W_NN_SCHEDULE  = [0.0, 0.3, 0.4, 0.4, 0.4, 0.4]   
W_NA_SCHEDULE  = [0.0, 0.2, 0.1, 0.05, 0.0, 0.0]  
W_DISTILL      = 1.0  # Weight for Relational Knowledge Distillation
...
class MemoryBank:
    """
    Class-balanced FIFO memory bank for old pseudo-classes.

    Same public API as the original:
        MemoryBank(size, dim, num_classes, device)
        enqueue(feats, labels, weights)
        get_all()

    Difference:
    - Separate FIFO queue per old pseudo-class.
    - Recency is preserved.
    - Easy classes cannot occupy the whole bank.
    """
    def __init__(self, size, dim, num_classes, device):
        ...
        self.per_class = max(1, int(np.ceil(size / num_classes)))
        self.size = self.per_class * num_classes

        self.features = torch.zeros(num_classes, self.per_class, dim, device=device)
        self.weights = torch.zeros(num_classes, self.per_class, device=device)
        self.valid = torch.zeros(num_classes, self.per_class, dtype=torch.bool, device=device)
        ...
        label_grid = torch.arange(num_classes, device=device).view(num_classes, 1)
        self.labels = label_grid.expand(num_classes, self.per_class)
    ...
    def enqueue(self, feats, labels, weights):
        ...
        for i in range(feats.size(0)):
            ...
            slot = int(self.ptrs[k].item())

            self.features[k, slot] = feats[i]
            self.weights[k, slot] = weights[i]
            self.valid[k, slot] = True

            self.ptrs[k] = (self.ptrs[k] + 1) % self.per_class
            self.counts[k] = torch.clamp(self.counts[k] + 1, max=self.per_class)
    ...
    def get_all(self):
        ...
        return flat_features[flat_valid], flat_labels[flat_valid], flat_weights[flat_valid]
...
# ──────────────────────────────────────────────────────────────────────────────
# SupCon Loss (Restored to Hard Margins for Purity)
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# Unsupervised Constrained Clustering (No Cheat, Margin Enabled)
# ──────────────────────────────────────────────────────────────────────────────
...
def make_constrained_joint_labels(Z_anchor_A, Z_anchor_B, hard_y_A, target_max_p=0.7):
    ...
    # ── Diagnostic plot (now threshold exists) ────────────────────────────────
    try:
        import matplotlib
        matplotlib.use('Agg')
        import matplotlib.pyplot as plt
        ...
        # True label overlay — only works when is_novel_B is accessible
        # Remove these two lines if running outside the main script scope
    ...
    # ── Silhouette Margin (20% boundary drop) to protect SupCon purity ───────
...
print("\n" + "="*72)
print("ITERATIVE REFINEMENT (Hard Margin + RKD + Class-Balanced High Purity MemBank)")
print("="*72)
