...
# ──────────────────────────────────────────────────────────────────────────────
# Baseline: normalize raw features → UMAP → normal Euclidean KMeans
# ──────────────────────────────────────────────────────────────────────────────
print("\nBaseline: L2-normalized raw features → combined UMAP-10 → normal KMeans …")
t0 = time.time()
X_AB_raw_norm = normalize(np.vstack([X_A, X_B]), norm="l2").astype(np.float32)
r_base    = umap.UMAP(n_components=10, n_neighbors=20, min_dist=0.05,
                      metric="cosine", random_state=SEED, verbose=False)
# IMPORTANT: do NOT L2-normalize the UMAP coordinates afterwards.
# UMAP defines an ordinary Euclidean coordinate system; the baseline now uses
# normal KMeans directly on these coordinates.
X_AB_base = r_base.fit_transform(X_AB_raw_norm).astype(np.float32)
...
# Eval helpers
# ──────────────────────────────────────────────────────────────────────────────
def gcd_acc(feat_B, tag="", n_init=20, verbose=True, normalize_before_kmeans=False):
    """
    Evaluate by normal Euclidean KMeans by default.

    Set normalize_before_kmeans=True only when intentionally evaluating in
    spherical/cosine geometry.  The updated baseline path is:
        raw → L2 normalize → UMAP → normal KMeans,
    so it calls this with normalize_before_kmeans=False.
    """
    km    = KMeans(n_clusters=K_NEW, n_init=n_init, random_state=SEED)
    feat_eval = normalize(feat_B, norm="l2") if normalize_before_kmeans else feat_B
    preds = km.fit_predict(feat_eval)
...
res_baseline = gcd_acc(X_B_base, "Baseline norm→UMAP-10→KMeans")
...
# ──────────────────────────────────────────────────────────────────────────────
def make_pseudo_labels(Z_A, target_max_p=0.7, normalize_input=True):
    """
    K-Means(K_OLD) pseudo-labels for A.

    normalize_input=True keeps the old spherical/cosine behavior:
        L2-normalize features → KMeans → normalize centroids → cosine soft scores.

    normalize_input=False is the updated UMAP baseline/bootstrap behavior:
        use the supplied coordinates as-is → normal Euclidean KMeans.
    This is intended for coordinates produced by:
        raw features → L2-normalize → UMAP.
    """
    if normalize_input:
        ...
        sim     = (Z_n @ centers.T).astype(np.float32)        # cosine scores
        hard    = sim.argmax(axis=1).astype(np.int64)
    else:
        Z_n = np.asarray(Z_A, dtype=np.float32)
        km  = KMeans(n_clusters=K_OLD, n_init=15, random_state=SEED).fit(Z_n)
        centers = km.cluster_centers_.astype(np.float32)
        hard    = km.labels_.astype(np.int64)
        # Convert Euclidean distances to similarity scores for soft pseudo-probs.
        # Negative squared distance preserves normal-KMeans geometry.
        d2      = ((Z_n[:, None, :] - centers[None, :, :]) ** 2).sum(axis=2)
        sim     = (-d2).astype(np.float32)
...
all_results = [("Baseline (norm→UMAP-10→KMeans)", *res_baseline)]
...
hard_y, soft_p, centers_A, used_T, ach_max = make_pseudo_labels(X_A_umap, target_max_p=0.7, normalize_input=False)
